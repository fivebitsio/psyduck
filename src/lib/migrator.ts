...
class Migrator {
  ...
  private appliedMigrationsQuery = 'select name from schema_migrations order by name'

  private deleteMigrationQuery = 'delete from schema_migrations where name = $1'
  private insertMigrationQuery = 'insert into schema_migrations (name, applied_at) values ($1, $2)'
  ...
  async up(): Promise<void> {
    ...
    for (const migration of migrations) {
      const query = await fs.readFile(path.join(this.migrationsDir, `${migration}.up.sql`), 'utf8')
      await this.executeQuery(query)
      await this.executeQuery(this.insertMigrationQuery, [migration, new Date().toISOString()])
  ...
  async down(numberOfMigrations: number | 'all'): Promise<void> {
    ...
    if (numberOfMigrations === 'all') {
      ...
      migrationsToRollback = appliedMigrations.slice(-numberOfMigrations).reverse()
    }
  ...
  private async getAppliedMigrations(): Promise<string[]> {
    try {
      ...
      console.error('Error fetching applied migrations:', (err as Error).message)
      throw err
    }
  }

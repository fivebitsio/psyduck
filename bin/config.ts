...
async function ensureUserExists(configService: ReturnType<typeof createConfigService>) {
  try {
    const userNames = await configService.listUserNames();
    ...
    if (!userNames || userNames.length === 0) {
      ...
      console.log(`Found ${userNames.length} existing user(s): ${userNames.join(', ')}`);
    }
  } catch (error) {
...
async function initializeConfig() {
  try {
    const configDb = await JSONFilePreset(configPath, { jwtKey: "", users: [] } as ConfigSchema);
